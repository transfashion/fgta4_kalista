...
var GaugeController = Chart.controllers.doughnut.extend({
  ...
  */
  ...
  drawTargetLine: function drawTargetLine(ease) {
    if (!this.chart.animating) {
		// triggered when hovering
		ease = 1;
	}
	...
	var _this$chart = this.chart,
		...
		config = _this$chart.config;
	var dataset = config.data.datasets[this.index];
	var targetLineValue = config.data.target;
	...
	var maxValue = this.getMaxValue(dataset.data);
	var targetPortion = targetLineValue/maxValue;
	var angle = (0.5 * Math.PI) + (Math.PI * targetPortion);	
