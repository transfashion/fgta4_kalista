...
/**
 * fgta/example/editorial/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header editorial (exm_editorial)
 ...
 * tanggal 25/02/2023
 */
$API = new class extends editorialBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\editorial_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new editorial_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.editorial_id LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'editorial_id' => 'A.`editorial_id`', 'editorial_title' => 'A.`editorial_title`', 'editorial_content' => 'A.`editorial_content`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "exm_editorial A";
			$sqlWhere = $where->sql;
