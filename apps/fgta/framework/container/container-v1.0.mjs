...
export async function init() {
	...
	pnl_iframe.on("load", function() {
		...
		if (modulefullname===undefined) {
			...
			fgta_output_content.html(pnl_iframe[0].contentDocument.body.innerHTML);
			$ui.ShowErrorWindow();
			return
		}
		...
		Cookies.set('last_opened_module', modulefullname, {path: window.urlparams.cookiepath})
		Cookies.set('last_opened_module_variance', variancename, {path: window.urlparams.cookiepath})
		Cookies.set('last_opened_module_urlparam', url_param, {path: window.urlparams.cookiepath})
		if (typeof pnl_iframe.frameloaded === 'function') {
			pnl_iframe.frameloaded()
		}
...
function open_last_module() {
	let last_opened_module = Cookies.get('last_opened_module', {path: window.urlparams.cookiepath});
	let last_opened_module_variance = Cookies.get('last_opened_module_variance', {path: window.urlparams.cookiepath});
	let last_opened_module_urlparam = Cookies.get('last_opened_module_urlparam', {path: window.urlparams.cookiepath});
	if (last_opened_module!=null) {
		...
		// open dashboard
		if (global.setup.dash_module!=null) {
			last_opened_module = global.setup.dash_module;
...
export function OpenModule(module, fn_loaded) {
	console.log(`opening ${module.modulefullname} module ...`);
	...
	// CurrentState.ErrorLoading = true
	// CurrentState.SlowLoading = true
	// setTimeout(()=>{
	// 	// load halaman timeout
	// 	fgta_output_content.html(pnl_iframe[0].contentDocument.body.innerHTML);
	// 	$ui.ShowErrorWindow();
	// 	$ui.unmask();
	// }, 15*1000);
	...
	pnl_iframe.frameunloaded = () => {
		Cookies.remove('last_opened_module', {path: window.urlparams.cookiepath});
		Cookies.remove('last_opened_module_variance', {path: window.urlparams.cookiepath});
		Cookies.remove('last_opened_module_urlparam', {path: window.urlparams.cookiepath});
