...
/**
 * fgta/framework/fgpermission/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header fgpermission (fgt_permission)
 *
 ...
 * tanggal 04/02/2023
 */
$API = new class extends fgpermissionBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fgt_permission';
		$primarykey = 'permission_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fgpermission_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new fgpermission_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->permission_name = strtoupper($obj->permission_name);
			...
			try {
				...
				$sqlFieldList = [
					'permission_id' => 'A.`permission_id`', 'permission_name' => 'A.`permission_name`', 'permission_descr' => 'A.`permission_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_permission A";
				$sqlWhere = $where->sql;
