...
const tbl_list = $('#pnl_editattachementgrid-tbl_list');
const txt_title = $('#pnl_editattachementgrid-title');
const pnl_control = $('.pnl_editattachementgrid-control');
const btn_removechecked  = $('#pnl_editattachementgrid-removechecked');
const btn_addrow = $('#pnl_editattachementgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.msg_subject)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/attachement-list`
		options.criteria['id'] = data.msg_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editattachementform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editattachementform', () => {
		$ui.getPages().ITEMS['pnl_editattachementform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/attachement-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editattachementform', ()=>{
		$ui.getPages().ITEMS['pnl_editattachementform'].handler.createnew(header_data)
	})	
}