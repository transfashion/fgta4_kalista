...
const tbl_list = $('#pnl_editcopygrid-tbl_list');
const txt_title = $('#pnl_editcopygrid-title');
const pnl_control = $('.pnl_editcopygrid-control');
const btn_removechecked  = $('#pnl_editcopygrid-removechecked');
const btn_addrow = $('#pnl_editcopygrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/copy-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcopyform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcopyform', () => {
		$ui.getPages().ITEMS['pnl_editcopyform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/copy-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcopyform', ()=>{
		$ui.getPages().ITEMS['pnl_editcopyform'].handler.createnew(header_data)
	})	
}