...
const tbl_list = $('#pnl_editfavegrid-tbl_list');
const txt_title = $('#pnl_editfavegrid-title');
const pnl_control = $('.pnl_editfavegrid-control');
const btn_removechecked  = $('#pnl_editfavegrid-removechecked');
const btn_addrow = $('#pnl_editfavegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/fave-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editfaveform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editfaveform', () => {
		$ui.getPages().ITEMS['pnl_editfaveform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/fave-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editfaveform', ()=>{
		$ui.getPages().ITEMS['pnl_editfaveform'].handler.createnew(header_data)
	})	
}