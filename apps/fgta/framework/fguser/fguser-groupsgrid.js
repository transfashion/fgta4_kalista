...
const tbl_list = $('#pnl_editgroupsgrid-tbl_list')
const txt_title = $('#pnl_editgroupsgrid-title')
const pnl_control = $('#pnl_editgroupsgrid-control')
const btn_removechecked  = $('#pnl_editgroupsgrid-removechecked')
const btn_addrow = $('#pnl_editgroupsgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/groups-list`
		options.criteria['id'] = data.user_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editgroupsform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editgroupsform', () => {
		$ui.getPages().ITEMS['pnl_editgroupsform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/groups-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editgroupsform', ()=>{
		$ui.getPages().ITEMS['pnl_editgroupsform'].handler.createnew(header_data)
	})	
}