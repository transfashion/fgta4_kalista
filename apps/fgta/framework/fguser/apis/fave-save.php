...
if (is_file(__DIR__ .'/data-fave-handler.php')) {
	require_once __DIR__ .'/data-fave-handler.php';
}
...
/**
 * fgta/framework/fguser/apis/fave-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel fave fguser (fgt_userfavemod)
 *
...
$API = new class extends fguserBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fgt_userfavemod';
		$primarykey = 'userfavemod_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fguser_faveHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fguser_faveHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"userfavemod_id" => $obj->userfavemod_id
				];

				$criteriaValues = [
					"userfavemod_id" => " userfavemod_id = :userfavemod_id "
				];
				...
				$sqlFieldList = [
					'userfavemod_id' => 'A.`userfavemod_id`', 'title' => 'A.`title`', 'modulefullname' => 'A.`modulefullname`', 'variancename' => 'A.`variancename`',
					'forecolor' => 'A.`forecolor`', 'backcolor' => 'A.`backcolor`', 'icon' => 'A.`icon`', 'user_id' => 'A.`user_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_userfavemod A";
				$sqlWhere = $where->sql;
