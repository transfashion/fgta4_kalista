...
/**
 * fgta/framework/fgrole/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header fgrole (fgt_role)
...
$API = new class extends fgroleBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\fgrole_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fgrole_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.role_id LIKE CONCAT('%', :search, '%') OR A.role_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'role_id' => 'A.`role_id`', 'role_name' => 'A.`role_name`', 'role_descr' => 'A.`role_descr`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "fgt_role A";
			$sqlWhere = $where->sql;
