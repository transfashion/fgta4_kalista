...
if (is_file(__DIR__ .'/data-permission-handler.php')) {
	require_once __DIR__ .'/data-permission-handler.php';
}
...
/**
 * fgta/framework/fgrole/apis/permission-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel permission fgrole (fgt_rolepermission)
 *
 ...
 * tanggal 04/02/2023
 */
$API = new class extends fgroleBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fgt_rolepermission';
		$primarykey = 'rolepermission_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fgrole_permissionHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fgrole_permissionHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->permission_id = strtoupper($obj->permission_id);
			...
			try {
				...
				$header_table = 'fgt_role';
				$header_primarykey = 'role_id';
				...
				// result
				$options->criteria = [
					"rolepermission_id" => $obj->rolepermission_id
				];

				$criteriaValues = [
					"rolepermission_id" => " rolepermission_id = :rolepermission_id "
				];
				...
				$sqlFieldList = [
					'rolepermission_id' => 'A.`rolepermission_id`', 'permission_id' => 'A.`permission_id`', 'role_id' => 'A.`role_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_rolepermission A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'permission_name' => \FGTA4\utils\SqlUtility::Lookup($record['permission_id'], $this->db, 'fgt_permission', 'permission_id', 'permission_name'),
