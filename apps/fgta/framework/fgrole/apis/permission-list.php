...
/**
 * fgta/framework/fgrole/apis/permission-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel permission fgrole (fgt_rolepermission)
...
$API = new class extends fgroleBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new fgrole_permissionHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('permission_name', 'permission_id', $record, 'fgt_permission', 'permission_name', 'permission_id');
