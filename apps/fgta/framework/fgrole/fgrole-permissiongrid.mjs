...
import * as hnd from  './fgrole-permissiongrid-hnd.mjs'

const tbl_list = $('#pnl_editpermissiongrid-tbl_list');
const txt_title = $('#pnl_editpermissiongrid-title');
const pnl_control = $('.pnl_editpermissiongrid-control');
const btn_removechecked  = $('#pnl_editpermissiongrid-removechecked');
const btn_addrow = $('#pnl_editpermissiongrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/permission-list`
		options.criteria['id'] = data.role_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpermissionform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpermissionform', () => {
		$ui.getPages().ITEMS['pnl_editpermissionform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/permission-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpermissionform', ()=>{
		$ui.getPages().ITEMS['pnl_editpermissionform'].handler.createnew(header_data)
	})	
}