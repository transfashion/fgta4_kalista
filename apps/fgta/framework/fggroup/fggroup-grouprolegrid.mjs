...
import * as hnd from  './fggroup-grouprolegrid-hnd.mjs'

const tbl_list = $('#pnl_editgrouprolegrid-tbl_list');
const txt_title = $('#pnl_editgrouprolegrid-title');
const pnl_control = $('.pnl_editgrouprolegrid-control');
const btn_removechecked  = $('#pnl_editgrouprolegrid-removechecked');
const btn_addrow = $('#pnl_editgrouprolegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/grouprole-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editgrouproleform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editgrouproleform', () => {
		$ui.getPages().ITEMS['pnl_editgrouproleform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/grouprole-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editgrouproleform', ()=>{
		$ui.getPages().ITEMS['pnl_editgrouproleform'].handler.createnew(header_data)
	})	
}