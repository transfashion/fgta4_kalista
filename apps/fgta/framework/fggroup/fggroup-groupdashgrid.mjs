...
import * as hnd from  './fggroup-groupdashgrid-hnd.mjs'

const tbl_list = $('#pnl_editgroupdashgrid-tbl_list');
const txt_title = $('#pnl_editgroupdashgrid-title');
const pnl_control = $('.pnl_editgroupdashgrid-control');
const btn_removechecked  = $('#pnl_editgroupdashgrid-removechecked');
const btn_addrow = $('#pnl_editgroupdashgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.group_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/groupdash-list`
		options.criteria['id'] = data.group_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editgroupdashform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editgroupdashform', () => {
		$ui.getPages().ITEMS['pnl_editgroupdashform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/groupdash-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editgroupdashform', ()=>{
		$ui.getPages().ITEMS['pnl_editgroupdashform'].handler.createnew(header_data)
	})	
}