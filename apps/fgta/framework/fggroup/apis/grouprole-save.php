...
/**
 * fgta/framework/fggroup/apis/grouprole-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel grouprole fggroup (fgt_grouprole)
 *
...
$API = new class extends fggroupBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new fggroup_grouproleHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"grouprole_id" => $obj->grouprole_id
				];

				$criteriaValues = [
					"grouprole_id" => " grouprole_id = :grouprole_id "
				];
				...
				$sqlFieldList = [
					'grouprole_id' => 'A.`grouprole_id`', 'role_id' => 'A.`role_id`', 'group_id' => 'A.`group_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_grouprole A";
				$sqlWhere = $where->sql;
