...
if (is_file(__DIR__ .'/data-groupdash-handler.php')) {
	require_once __DIR__ .'/data-groupdash-handler.php';
}
...
/**
 * fgta/framework/fggroup/apis/groupdash-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel groupdash fggroup (fgt_groupdash)
 *
...
$API = new class extends fggroupBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fgt_groupdash';
		$primarykey = 'groupdash_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fggroup_groupdashHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fggroup_groupdashHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"groupdash_id" => $obj->groupdash_id
				];

				$criteriaValues = [
					"groupdash_id" => " groupdash_id = :groupdash_id "
				];
				...
				$sqlFieldList = [
					'groupdash_id' => 'A.`groupdash_id`', 'dash_id' => 'A.`dash_id`', 'group_id' => 'A.`group_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fgt_groupdash A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'dash_name' => \FGTA4\utils\SqlUtility::Lookup($record['dash_id'], $this->db, 'fgt_dash', 'dash_id', 'dash_name'),
