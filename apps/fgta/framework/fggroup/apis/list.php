...
/**
 * fgta/framework/fggroup/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header fggroup (fgt_group)
...
$API = new class extends fggroupBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\fggroup_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fggroup_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.group_id LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'group_id' => 'A.`group_id`', 'group_name' => 'A.`group_name`', 'group_descr' => 'A.`group_descr`', 'group_disabled' => 'A.`group_disabled`',
				'group_menu' => 'A.`group_menu`', 'dash_id' => 'A.`dash_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "fgt_group A";
			$sqlWhere = $where->sql;
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('dash_name', 'dash_id', $record, 'fgt_dash', 'dash_name', 'dash_id');
