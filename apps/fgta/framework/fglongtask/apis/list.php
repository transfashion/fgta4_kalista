...
/**
 * fgta/framework/fglongtask/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header fglongtask (fgt_longtask)
 ...
 * tanggal 08/07/2023
 */
$API = new class extends fglongtaskBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\fglongtask_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new fglongtask_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.longtask_id LIKE CONCAT('%', :search, '%') OR A.longtask_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'longtask_id' => 'A.`longtask_id`', 'longtask_name' => 'A.`longtask_name`', 'longtask_start' => 'A.`longtask_start`', 'longtask_expired' => 'A.`longtask_expired`',
				'longtask_progress' => 'A.`longtask_progress`', 'longtask_taskdescr' => 'A.`longtask_taskdescr`', 'longtask_logfile' => 'A.`longtask_logfile`', 'longtask_lastprogressid' => 'A.`longtask_lastprogressid`',
				'longtask_isrunning' => 'A.`longtask_isrunning`', 'longtask_isrequestcancel' => 'A.`longtask_isrequestcancel`', 'longtask_iscanceled' => 'A.`longtask_iscanceled`', 'longtask_iscomplete' => 'A.`longtask_iscomplete`',
				'longtask_iserror' => 'A.`longtask_iserror`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "fgt_longtask A";
			$sqlWhere = $where->sql;
