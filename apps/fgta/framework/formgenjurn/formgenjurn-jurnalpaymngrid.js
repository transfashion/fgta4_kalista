...
const tbl_list = $('#pnl_editjurnalpaymngrid-tbl_list')
const txt_title = $('#pnl_editjurnalpaymngrid-title')
const pnl_control = $('#pnl_editjurnalpaymngrid-control')
const btn_removechecked  = $('#pnl_editjurnalpaymngrid-removechecked')
const btn_addrow = $('#pnl_editjurnalpaymngrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/jurnalpaymn-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editjurnalpaymnform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editjurnalpaymnform', () => {
		$ui.getPages().ITEMS['pnl_editjurnalpaymnform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/jurnalpaymn-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editjurnalpaymnform', ()=>{
		$ui.getPages().ITEMS['pnl_editjurnalpaymnform'].handler.createnew(header_data)
	})	
}