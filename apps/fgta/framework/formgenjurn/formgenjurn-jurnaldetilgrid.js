...
const tbl_list = $('#pnl_editjurnaldetilgrid-tbl_list')
const txt_title = $('#pnl_editjurnaldetilgrid-title')
const pnl_control = $('#pnl_editjurnaldetilgrid-control')
const btn_removechecked  = $('#pnl_editjurnaldetilgrid-removechecked')
const btn_addrow = $('#pnl_editjurnaldetilgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/jurnaldetil-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editjurnaldetilform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editjurnaldetilform', () => {
		$ui.getPages().ITEMS['pnl_editjurnaldetilform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/jurnaldetil-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editjurnaldetilform', ()=>{
		$ui.getPages().ITEMS['pnl_editjurnaldetilform'].handler.createnew(header_data)
	})	
}